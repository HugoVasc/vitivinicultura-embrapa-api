...
from ..models import WineCategories, ComercializedWineCategoriesWithQuantity, WineSubCategories, ComercializedWineSubCategoriesWithQuantity
...
async def comercializacao():
    ...
    for item in result:
        ...
        response[str(item.id)] = {
            ...
            "ano": item.year,
            "quantidade_litros": item.quantity_in_l
        }
...
async def comercializacao_categoria(categoria: str = "VINHO DE MESA"):
    """
    ...
    #     WineCategories, ProducedWineCategoriesWithQuantity.category == WineCategories.id
        ...
        ComercializedWineCategoriesWithQuantity.category == categoria
    ).all()
    return format_response(result)

@router.get("/categoria/lista", status_code=status.HTTP_200_OK)
async def comercializacao_categoria_lista():
    """
...
async def comercializacao_subcategoria_lista():
    """
...
async def comercializacao_subcategoria():
    """
    Rota para /producao/subcategoria. 
    Lista todas os itens de subcategoria.
    """
    ...
    result = SessionLocal().query(
        ...
        WineSubCategories, ComercializedWineSubCategoriesWithQuantity.subcategory_id == WineSubCategories.id
    ).all()
    ...
    for item in result:
        ...
        response[str(item[0].id)] = {
            ...
            "ano": item[0].year,
            "quantidade_litros": item[0].value
        }
